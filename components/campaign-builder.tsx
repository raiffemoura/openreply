...
export default function CampaignBuilder({ mode, campaignId }: CampaignBuilderProps) {
  ...
  async function handleSubmit(activeValue: boolean) {
    ...
    if (matchMode === "specific" && keywords.length === 0)
      return setError("Adicione ao menos uma palavra-chave, ou mude para qualquer palavra.");
    ...
    if (openingDmEnabled && (!openingDmMessage.trim() || !openingDmButtonLabel.trim()))
      return setError("A DM de abertura precisa de uma mensagem e um rótulo de botão.");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-8">
        ...
        <Section title="E o comentário tiver">
          ...
          <div className="flex items-center justify-between gap-3 rounded-lg border border-border px-3 py-2.5">
            <span className="text-sm text-foreground">
              também responder quando mandarem DM com{" "}
          ...
          {dmTriggerEnabled && (
            <p className="text-xs text-muted">
              {matchMode === "any"
                ? "Toda DM para essa conta recebe a resposta abaixo — use com cuidado."
                : "Uma DM com qualquer uma dessas palavras recebe a mesma resposta, sem precisar comentar."}
            </p>
          )}
          <div className="flex items-center justify-between rounded-lg border border-border px-3 py-2.5">
            <span className="text-sm text-foreground">
              responder ao comentário dela embaixo do post
            </span>
            <Toggle
          ...
          {publicReplyEnabled && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted">
                Uma é escolhida ao acaso a cada vez, para as respostas não
                ficarem idênticas.
              </p>
            </div>
          )}
            ...
            {requireFollow && (
              <div className="mt-3 space-y-2">
                <textarea
                  ...
                  placeholder="Um favor antes de te mandar o link: me seguir aqui já ajuda demais 🙏. Assim que estiver me seguindo, toca no botão que eu te envio na hora."
                  rows={3}
                  className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none resize-none"
                  maxLength={1000}
                />
                ...
                />
                <p className="text-xs text-muted">
                  Só enviamos o link depois que ela toca no botão e o Instagram
                  confirma o follow. Se não der para verificar, enviamos mesmo
                  assim.
                </p>
              </div>
            )}
        ...
        <Section title="E depois, ela vai receber">
            ...
            {followUpEnabled && (
              <div className="mt-3 space-y-2">
                <textarea
                  ...
                  placeholder="Ah, e valeu por me seguir, viu? Fico muito feliz com o seu apoio 🙌"
                  rows={3}
                  className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none resize-none"
                  maxLength={1000}
                />
                ...
                <p className="text-xs text-muted">
                  {followUpDelayMinutes > 0
                    ? `Enviada ${followUpDelayMinutes} min depois que ela abrir o link.`
                    : "Enviada logo depois que ela abrir o link."}
                  {" {username}"} personaliza. Máximo de 24 horas, para ficar
